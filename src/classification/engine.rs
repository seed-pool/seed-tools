...
impl MediaClassification {
    ...
    fn classify_with_media_module_static(
        ...
        let (category, source_type) = match media_type {
            MediaType::Video(_) => {
                let cat = rules::classify_video_rules(&enriched_metadata);
                let src = rules::classify_video_source_type(&enriched_metadata);
                (cat, src)
            }
            MediaType::Audio(_) => {
                let cat = rules::classify_audio_rules(&enriched_metadata);
                let src = rules::classify_audio_source_type(&enriched_metadata);
                (cat, src)
            }
            MediaType::Ebook(_) => {
                let cat = rules::classify_ebook_rules(&enriched_metadata);
                (cat, None) // Ebooks don't have source types
            }
            MediaType::Game(_) => {
                let cat = rules::classify_game_rules(&enriched_metadata);
                (cat, None) // Games don't have source types
            }
            MediaType::Hobby(_) => {
                let cat = rules::classify_hobby_rules(&enriched_metadata);
        ...
        if category.is_some() {
            ...
            // Fallback to media module classification
            match media_type {
                MediaType::Video(_) => {
