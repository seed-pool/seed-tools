...
use crate::core::{
    ...
    types::{Config, SeedpoolConfig, TorrentLeechConfig},
};
use log::info;
use std::fs;
...
pub fn check_duplicates(
    ...
    match tracker.to_lowercase().as_str() {
        "seedpool" | "sp" => {
            let seedpool_cfg = seedpool_config.ok_or(SeedError::Config(
                "Seedpool configuration is required for Seedpool duplicate checks".to_string(),
            ))?;
        ...
        "torrentleech" | "tl" => {
            let _tl_cfg = torrentleech_config.ok_or(SeedError::Config(
                "TorrentLeech configuration is required for TorrentLeech duplicate checks"
                    .to_string(),
            ))?;
        ...
        _ => Err(SeedError::Config(format!("Unknown tracker: {}", tracker))),
    }
}
