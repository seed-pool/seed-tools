...
impl BinaryManager {
    ...
    pub async fn check_binaries(&self) -> Result<Vec<&'static str>, String> {
        ...
        for binary in &required_binaries {
            ...
            if self
                .is_binary_working(&binary_path, binary.verify_args)
                .await
            {
                continue;
            }
            ...
            if binary.name == "mediainfo" {
                if self
                    .is_system_binary_working("mediainfo", binary.verify_args)
                    .await
                {
    ...
    pub async fn install_missing_binaries(&self, missing: &[&str]) -> Result<(), String> {
        ...
        println!(
            "🚀 Setting up {} required binaries for first-time use...",
            missing.len()
        );
    ...
    async fn download_and_install_binary(&self, binary: &BinaryInfo) -> Result<(), String> {
        ...
        let archive_path = self
            .bin_dir
            .join(format!("{}.{}", binary.name, archive_extension));
        ...
        while let Some(chunk) = stream.next().await {
            ...
            if total_size > 0 && downloaded % 5242880 == 0 {
                // Every 5MB
        ...
        match extract_single_archive(&archive_path, &temp_extract_dir) {
            Ok(_) => {
                ...
                self.find_and_copy_binary(&temp_extract_dir, binary, &final_path)
                    .await?;
        ...
        if !self
            .is_binary_working(&final_path, binary.verify_args)
            .await
        {
            fs::remove_file(&final_path).ok();
            return Err(format!(
                "Downloaded {} binary is not working correctly",
                binary.name
            ));
        }
    ...
    async fn find_and_copy_binary(
        ...
        if binary.name == "mediainfo" {
            ...
            for source_dir in &source_dirs {
                if source_dir.exists() {
                    ...
                    return self
                        .build_mediainfo_with_dependencies(source_dir, final_path)
                        .await;
                }
            }
        }
                ...
                if filename == target_name {
                    ...
                    if path.is_file() {
                        ...
                        if absolute_target.is_file() {
                            ...
                            return Err(format!(
                                "Symlink target does not exist or is not a file: {:?}",
                                absolute_target
                            ));
                        }
                    } else {
                        return Err(format!(
                            "Found binary is neither a regular file nor a symlink: {:?}",
                            path
                        ));
                    }
        ...
        info!(
            "❌ Binary '{}' not found. Files found in archive:",
            target_name
        );
        for entry in walkdir::WalkDir::new(search_dir).max_depth(3) {
            if let Ok(entry) = entry {
                ...
                if path.is_file() {
                    info!(
                        "  📄 {}",
                        path.strip_prefix(search_dir).unwrap_or(path).display()
                    );
                }
            }
        }
    ...
    async fn build_mediainfo_with_dependencies(
        ...
        info!(
            "🔨 Building MediaInfo with dependencies from: {:?}",
            source_root
        );
        ...
        self.download_and_build_libzen(&build_dir, &libzen_prefix)
            .await?;
        ...
        self.download_and_build_libmediainfo(&build_dir, &libzen_prefix, &libmediainfo_prefix)
            .await?;
        ...
        let output = tokio::process::Command::new("./configure")
            ...
            .env(
                ...
                format!(
                    "-L{}/lib -L{}/lib",
                    libzen_prefix.display(),
                    libmediainfo_prefix.display()
                ),
            )
            .output()
            .await
    ...
    async fn download_and_build_libzen(
        ...
        if !response.status().is_success() {
            return Err(format!(
                "Failed to download libzen: HTTP {}",
                response.status()
            ));
        }
    ...
    async fn download_and_build_libmediainfo(
        ...
        if !response.status().is_success() {
            return Err(format!(
                "Failed to download libmediainfo: HTTP {}",
                response.status()
            ));
        }
        ...
        crate::processing::extraction::extract_single_archive(
            &libmediainfo_archive,
            &libmediainfo_extract_dir,
        )
        ...
        if autogen_path.exists() {
            ...
            if !output.status.success() {
                return Err(format!(
                    "libmediainfo autogen.sh failed: {}",
                    String::from_utf8_lossy(&output.stderr)
                ));
            }
        }
