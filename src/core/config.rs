...
use super::error::{SeedError, Result};
...
pub fn validate_config(config: &Config) -> Result<()> {
    ...
    for (name, path) in &binaries {
        if !Path::new(path).exists() {
            return Err(SeedError::Config(format!("Binary '{}' not found at '{}'", name, path)));
        }
    }
    
    // Validate directories exist
    if !Path::new(&config.paths.torrent_dir).exists() {
        return Err(SeedError::Config(format!("Torrent directory not found: {}", config.paths.torrent_dir)));
    }
    
    if !Path::new(&config.paths.screenshots_dir).exists() {
        return Err(SeedError::Config(format!("Screenshots directory not found: {}", config.paths.screenshots_dir)));
    }
