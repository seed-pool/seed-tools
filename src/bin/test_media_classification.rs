...
fn scan_directory(dir_path: &str, media_type: &TestMediaType) -> Result<Vec<String>, String> {
    ...
    let extensions = match media_type {
        TestMediaType::Video => vec![
            "mkv", "mp4", "avi", "mov", "wmv", "flv", "webm", "m4v", "ts", "mpg", "mpeg", "iso",
        ],
        TestMediaType::Audio => vec![
            "mp3", "flac", "wav", "aac", "ogg", "m4a", "wma", "aiff", "ape", "opus",
        ],
        TestMediaType::Ebook => vec![
            "epub", "pdf", "mobi", "azw", "azw3", "cbr", "cbz", "lit", "pdb",
        ],
        TestMediaType::Game => vec!["zip", "rar", "7z", "exe", "msi", "iso", "nsp", "xci", "cia"],
        TestMediaType::Hobby => vec![
            "doc", "docx", "txt", "rtf", "jpg", "png", "gif", "zip", "rar", "csv", "json", "xml",
        ],
    };
...
fn generate_random_filenames(media_type: &TestMediaType, count: usize) -> Vec<String> {
    ...
    match media_type {
        TestMediaType::Video => {
            ...
            let years = [
                "1999", "2005", "2010", "2015", "2019", "2020", "2021", "2022", "2023", "2024",
            ];
            let resolutions = ["480p", "720p", "1080p", "2160p", "4K", "576p", "1080i"];
            let sources = [
                "BluRay", "WEB-DL", "HDTV", "DVDRip", "WEBRip", "BRRip", "HDRip", "AMZN", "NF",
                "HMAX",
            ];
            let codecs = ["x264", "x265", "H264", "HEVC", "XviD", "DivX", "VP9", "AV1"];
            let groups = [
                "SPARKS", "DEMAND", "NTb", "RARBG", "YTS", "FGT", "EZTV", "LOL", "FLEET",
                "SYNCOPY", "MeGusta", "ION10", "REiGN", "AFG",
            ];
            ...
            filenames.push(
                "Sports/NFL.2024.Week.01.Cowboys.vs.Giants.1080p.WEB.h264-SPORTSNET.mkv"
                    .to_string(),
            );
            filenames.push(
                "Complete.Series/Friends.Complete.Series.1080p.BluRay.x264/Friends.S01E01.mkv"
                    .to_string(),
            );
            filenames.push(
                "The.Office.US.S09E23-E24.Finale.1080p.WEB-DL.DD5.1.H.264-BS.mkv".to_string(),
            );
        }
        ...
        TestMediaType::Audio => {
            ...
            let formats = [
                "FLAC", "MP3 320", "MP3 V0", "AAC", "24-96", "DSD", "24-192", "16-44", "MP3 192",
                "M4A", "OPUS",
            ];
            let sources = [
                "CD",
            ...
            filenames
                .push("Joe Rogan Experience #1999 - Guest Name [MP3]/jre-1999.mp3".to_string());
            filenames.push("test.mp3".to_string());
            filenames
                .push("Podcasts/The Tim Ferriss Show/Episode 500 - Special Guest.mp3".to_string());
        ...
        TestMediaType::Ebook => {
            ...
            let years = [
                "1985", "1990", "1995", "2000", "2005", "2010", "2015", "2020", "2021", "2022",
                "2023", "2024",
            ];
            let publishers = [
                "Penguin",
            ...
            for _ in 0..count {
                ...
                let is_comic = title.contains("Spider-Man")
                    || title.contains("Batman")
                    || title.contains("X-Men")
                    || title.contains("Walking Dead")
                    || title.contains("Superman")
                    || title.contains("Wonder Woman")
            ...
            filenames.push(
                "Comics/Marvel/Avengers (2018)/Avengers 001 (2018) (Digital).cbz".to_string(),
            );
            filenames
                .push("Image Comics - Saga 001-054 (2012-2018)/Saga 001 (2012).cbr".to_string());

            // Technical books with folders
            filenames.push(
                "Programming/Robert C Martin - Clean Code (2008) [PDF]/Clean Code.pdf".to_string(),
            );
            filenames.push(
                "IT Books/The Pragmatic Programmer - From Journeyman to Master.epub".to_string(),
            );

            // Fiction with series info
            filenames.push(
                "Fantasy/Brandon Sanderson/Mistborn 01 - The Final Empire (2006).epub".to_string(),
            );
            filenames.push(
                "George R.R. Martin - A Song of Ice and Fire/Book 1 - A Game of Thrones.mobi"
                    .to_string(),
            );
        }
        ...
        TestMediaType::Game => {
            ...
            let platforms = [
                "PC", "PS4", "PS5", "Xbox", "Switch", "Windows", "MacOS", "Linux",
            ];
            let years = ["2020", "2021", "2022", "2023", "2024"];
            let groups = [
                "CODEX", "PLAZA", "FitGirl", "DODI", "GOG", "Steam", "Repack",
            ];
