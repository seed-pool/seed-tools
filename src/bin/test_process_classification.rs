...
fn main() {
    ...
    let active_tracker = if seedpool_config.as_ref().map(|c| c.general.enabled).unwrap_or(false) {
        Some("seedpool")
    } else if torrentleech_config.as_ref().map(|c| c.general.enabled).unwrap_or(false) {
        Some("torrentleech")
    } else {
        None
    };
    ...
    if test_paths.is_empty() || (args.count > 0 && args.file.is_none() && args.directory.is_none() && !had_direct_input) {
...
fn create_temp_test_directory(media_type: &MediaType) -> String {
    let temp_dir = format!("/tmp/seed-tools-test-classification-{}-{}", 
        match media_type {
...
fn generate_test_paths(temp_dir: &str, media_type: &MediaType, count: usize) -> Vec<String> {
    ...
    match media_type {
        MediaType::Video => {
            let titles = ["Breaking.Bad", "The.Wire", "Game.of.Thrones", "Inception", "The.Matrix", 
                         "Stranger.Things", "The.Dark.Knight", "Interstellar", "Attack.on.Titan"];
            ...
            for _ in 0..count {
                ...
                if rng.gen_bool(0.3) {
                    let dir_path = format!("{}/{}", temp_dir, filename.replace(".mkv", "").replace(".mp4", ""));
        ...
        MediaType::Audio => {
            let artists = ["Pink Floyd", "Led Zeppelin", "The Beatles", "Nirvana", "Radiohead",
                          "Various Artists", "VA", "Podcast", "Audiobook Collection"];
        ...
        MediaType::Ebook => {
            let authors = ["Stephen King", "J.K. Rowling", "George R.R. Martin", "Marvel Comics",
                          "DC Comics", "National Geographic", "O'Reilly"];
        ...
        MediaType::Game => {
            let games = ["Cyberpunk.2077", "The.Witcher.3", "GTA.V", "Red.Dead.Redemption.2",
                        "Call.of.Duty.Modern.Warfare", "FIFA.24", "NBA.2K24"];
        ...
        MediaType::Hobby => {
            let types = ["Tutorial", "Template", "Resource.Pack", "Collection", "Assets", "Fonts"];
    ...
    match media_type {
        MediaType::Video => {
            ...
            paths.push(format!("{}/[HorribleSubs] One Piece - 1000 [1080p].mkv", temp_dir));
            paths.push(format!("{}/NFL.2024.Week.01.Cowboys.vs.Giants.1080p.WEB.h264-SPORTSNET.mkv", temp_dir));
        }
        MediaType::Audio => {
            paths.push(format!("{}/test.mp3", temp_dir));
            paths.push(format!("{}/Joe Rogan Experience #2000 - Elon Musk.mp3", temp_dir));
        }
        MediaType::Ebook => {
        ...
        MediaType::Hobby => {
            ...
            paths.push(format!("{}/1000.Premium.Fonts.Collection.2024.rar", temp_dir));
        }
    }
    ...
    for path in &paths {
        if !Path::new(path).exists() {
            if path.ends_with(".mp4") || path.ends_with(".mkv") || path.ends_with(".mp3") || 
               path.ends_with(".epub") || path.ends_with(".pdf") || path.ends_with(".exe") || 
               path.ends_with(".zip") || path.ends_with(".rar") {
