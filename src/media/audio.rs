use crate::core::types::{
    AudioCategory, AudioFile, AudioSourceType, AudioType, MediaFile, MediaType,
};
...
pub fn process_audio(
    ...
    if !path.exists() {
        ...
        info!("Path '{}' does not exist, returning empty audio results for preflight mode", input_path);
        return Ok(Vec::new());
    }
    ...
    if path.is_file() {
            ...
            if file_path.is_file() {
                if let Some(extension) = file_path.extension().and_then(|ext| ext.to_str()) {
                    if let Some(audio_type) = AudioType::from_extension(extension) {
                        ...
                        info!(
                            "Processed audio: {} -> Category: {:?}, Source: {:?}",
                            file_path.display(),
                            metadata.category,
        ...
        if !rejected_files.is_empty() {
            warn!(
                "Processed {} valid audio files, rejected {} files with unknown categories",
                results.len(),
                rejected_files.len()
            );
        }
    ...
    if !results.is_empty() {
        ...
        if let Some(album) = &metadata.album {
            builder =
                builder.with_title_info(album, metadata.year.map(|y| y.to_string()).as_deref());
        }
        ...
        builder = builder
            ...
            .with_custom_component(
                "audio_metadata",
                crate::core::UploadComponent::Metadata(audio_metadata),
            );
...
fn detect_audio_files_recursive(
    path: &Path,
...
pub fn classify_audio_content(path: &Path, audio_type: &AudioType) -> AudioMetadata {
    ...
    let live_regex = Regex::new(
        r"(?i)\b(live\s+at|live\s+from|live\s+in|concert|unplugged|acoustic\s+live|bootleg)\b",
    )
    .unwrap();
    let bootleg_regex = Regex::new(r"(?i)\b(bootleg|unofficial|rare|unreleased|demo)\b").unwrap();
    let compilation_regex = Regex::new(
        r"(?i)\b(compilation|best\s+of|greatest\s+hits|anthology|collection|selected)\b",
    )
    .unwrap();
    ...
    let audiobook_regex =
        Regex::new(r"(?i)\b(audiobook|audio\s+book|narrated|unabridged|abridged)\b").unwrap();
...
pub fn classify_for_upload(
    ...
    // Default to music
    Ok((
        Some("AudioCategory::Music".to_string()),
        None,
        metadata.clone(),
    ))
}
...
pub fn generate_description_with_enriched_metadata(
    ...
    use crate::core::{DescriptionComponent, ImageLayout, MediaType, SectionFormat};
    ...
    let mut builder =
        DescriptionBuilder::with_config(MediaType::Audio(audio_metadata.format.clone()), config);
    ...
    let artist_to_use =
        get_value("musicbrainz_artist").or_else(|| audio_metadata.artist.as_deref());
    ...
    let cover_images: Vec<String> = base_metadata
        .get("cover_images")
    ...
    metadata_rows.push(vec![
        "Category".to_string(),
        format!("{:?}", audio_metadata.category),
    ]);
    ...
    let label_to_use = get_value("musicbrainz_labels").or_else(|| audio_metadata.label.as_deref());
    ...
    metadata_rows.push(vec![
        "Source".to_string(),
        format!("{:?}", audio_metadata.source_type),
    ]);
    ...
    {
        let tracklist_string = tracklist_array
            .iter()
...
fn extract_audio_metadata_from_json(metadata: &serde_json::Value) -> AudioMetadata {
    AudioMetadata {
        ...
        sample_rate: metadata
            ...
            .and_then(|s| s.as_str())
            .map(String::from),
        is_various_artists: metadata
            .get("is_various_artists")
