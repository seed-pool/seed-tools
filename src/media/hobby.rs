use crate::core::types::{HobbyFile, HobbyType, MediaFile, MediaType, HobbyCategory};
use std::path::Path;
use log::{info, warn};
use regex::Regex;
...
pub fn process_hobby(
    ...
    if !results.is_empty() {
        ...
        builder = builder
            ...
            .with_custom_component("hobby_metadata", crate::core::UploadComponent::Metadata(hobby_metadata));
...
pub fn classify_for_upload(input_path: &str, metadata: &serde_json::Value) -> Result<(Option<String>, Option<String>, serde_json::Value), String> {
    ...
    if let Some(category) = metadata.get("category").and_then(|c| c.as_str()) {
        ...
        let fields_to_check = ["title", "author", "version", "language", "images", "instructions", "requirements", "usage_notes"];
        ...
        return Ok((Some(format!("HobbyCategory::{}", category)), None, enriched_metadata));
    }
    ...
    if let Ok(hobby_files) = detect_hobby_files(input_path) {
        if let Some(hobby_file) = hobby_files.first() {
            let filename = hobby_file.path.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("");
    ...
    Ok((Some("HobbyCategory::General".to_string()), None, default_metadata))
}
...
pub fn generate_description(metadata: &serde_json::Value) -> String {
    ...
    use crate::core::{MediaType, HobbyType, ImageLayout, SectionFormat, DescriptionComponent};
    ...
    let images: Vec<String> = metadata.get("images")
    ...
    if let Some(total_size_mb) = metadata.get("total_size_mb").and_then(|s| s.as_u64()) {
        info_rows.push(vec!["Total Size".to_string(), format!("{} MB", total_size_mb)]);
    }
    ...
    if let Some(instructions) = metadata.get("instructions").and_then(|i| i.as_str()) {
        builder = builder.custom_section("Installation Instructions", instructions, SectionFormat::Quoted);
    }
