...
pub fn search_igdb_game(
    ...
    if !response.status().is_success() {
        ...
        return Err(SeedError::ApiError(format!("IGDB API error: {} - {}", status, error_text)));
    }
    
    let response_text = response.text()
        .map_err(|e| SeedError::ApiError(format!("Failed to read IGDB response: {}", e)))?;
    
    let games: Vec<Value> = serde_json::from_str(&response_text)
        .map_err(|e| SeedError::ApiError(format!("Failed to parse IGDB response: {} - Response was: {}", e, response_text)))?;
...
pub fn get_igdb_game_details(
    ...
    if !response.status().is_success() {
        return Err(SeedError::ApiError(format!("IGDB API error: {}", response.status())));
    }
    
    let mut games: Vec<Value> = response.json()
...
pub fn extract_igdb_metadata(igdb_details: &Value) -> std::collections::HashMap<String, String> {
    ...
    if let Some(game_modes) = igdb_details["game_modes"].as_array() {
        let mode_names: Vec<String> = game_modes.iter()
    ...
    if let Some(themes) = igdb_details["themes"].as_array() {
        let theme_names: Vec<String> = themes.iter()
    ...
    if let Some(perspectives) = igdb_details["player_perspectives"].as_array() {
        let perspective_names: Vec<String> = perspectives.iter()
            .filter_map(|p| p["name"].as_str())
            .map(|s| s.to_string())
            .collect();
        if !perspective_names.is_empty() {
            metadata.insert("igdb_player_perspectives".to_string(), perspective_names.join(", "));
        }
    }

    // Franchises
    if let Some(franchises) = igdb_details["franchises"].as_array() {
        let franchise_names: Vec<String> = franchises.iter()
    ...
    if let Some(artworks) = igdb_details["artworks"].as_array() {
        let artwork_urls: Vec<String> = artworks.iter()
    ...
    if let Some(websites) = igdb_details["websites"].as_array() {
        let website_urls: Vec<String> = websites.iter()
