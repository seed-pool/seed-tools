use crate::core::{
    DescriptionComponent, EbookType, ImageLayout, MediaType, SectionFormat, VideoType,
};
        ...
        for component in &self.components {
            let bbcode = match component {
                DescriptionComponent::Title { text, size, color } => {
                    format!(
                        "[center][b][size={}][color={}]{}[/color][/size][/b][/center]",
                        size, color, text
                    )
                }
                ...
                DescriptionComponent::Author { name, color } => {
                    format!(
                        "[center][b][size=16][color={}]By:[/color][/size][/b] [i]{}[/i][/center]",
                        color, name
                    )
                }

                DescriptionComponent::Images {
                    urls,
                    layout,
                    width,
                ...
                DescriptionComponent::Synopsis { text } => {
                    ...
                    format!(
                        "[b][size=15][color={}]Synopsis:[/color][/size][/b]\n[quote]{}[/quote]",
                        color, text
                    )
                }
                ...
                DescriptionComponent::Trailer { url, platform } => {
                    format!(
                        "[center][b][url={}][Trailer on {}][/url][/b][/center]",
                        url, platform
                    )
                }

                DescriptionComponent::CustomSection {
                    title,
                    content,
                    format,
                } => self.format_custom_section(title, content, format),

                DescriptionComponent::Table { rows } => self.format_table(rows),
...
pub fn create_video_description(
    ...
    let mut builder = DescriptionBuilder::new(MediaType::Video(VideoType::Mkv));
    ...
    if !screenshots.is_empty() {
        ...
        builder = DescriptionBuilder::with_config(MediaType::Video(VideoType::Mkv), config)
            .images(screenshots);
    }
...
pub fn create_ebook_description(
    ...
    let mut builder = DescriptionBuilder::new(MediaType::Ebook(EbookType::Epub)).title(title);
...
pub fn create_ebook_description_with_metadata(
    ...
    if !additional_links.is_empty() {
        let links_content = additional_links
            .iter()
