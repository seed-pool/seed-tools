            ...
            if let Ok(entry) = entry {
                ...
                if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                    if audio_extensions.contains(&ext.to_lowercase().as_str()) {
                        cover_path =
                            extract_embedded_artwork(&path, &output_dir, release_name, config)?;
...
fn extract_embedded_artwork(
    ...
    info!(
        "Attempting to extract embedded artwork from: {}",
        audio_file.display()
    );
    ...
    if output.status.success() && cover_path.exists() {
        ...
        if file_size > 0 {
            info!(
                "✅ Successfully extracted embedded cover art: {} bytes",
                file_size
            );
...
fn fetch_from_musicbrainz(
    ...
    if let Some(mbid) = release_id {
        info!(
            "Attempting to fetch cover art from MusicBrainz for release: {}",
            mbid
        );
