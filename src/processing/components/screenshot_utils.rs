...
use crate::utils::http::{upload_to_cdn, upload_to_imgbb};
use log::info;
use std::fs;
use std::path::Path;
...
pub fn generate_screenshots(
    ...
    let ffmpeg_path = _ffmpeg_path
        .to_str()
        .ok_or(SeedError::Validation("Invalid ffmpeg path".to_string()))?;
    let ffprobe_path_str = ffprobe_path
        .to_str()
        .ok_or(SeedError::Validation("Invalid ffprobe path".to_string()))?;
    ...
    if has_imgbb {
        ...
        for (i, timestamp) in timestamps.iter().enumerate() {
            ...
            // Clean up temp file
            fs::remove_file(&screenshot_path).map_err(|e| {
                SeedError::Other(format!("Failed to delete temporary screenshot: {}", e))
            })?;
        }
    ...
    for (i, timestamp) in timestamps.iter().enumerate() {
        ...
        #[cfg(unix)]
        {
            ...
            fs::set_permissions(&screenshot_file, fs::Permissions::from_mode(0o777)).map_err(
                |e| {
            ...
            fs::set_permissions(&thumbnail_file, fs::Permissions::from_mode(0o777)).map_err(
                |e| {
                    SeedError::Other(format!(
                        "Failed to set permissions for {}: {}",
                        thumbnail_file, e
        ...
        if !dry_run {
            upload_to_cdn(
                &screenshot_file,
                &format!("{}/screenshots/", remote.trim_end_matches('/')),
            )?;
            upload_to_cdn(
                &thumbnail_file,
                &format!("{}/screenshots/", remote.trim_end_matches('/')),
        ...
        let screenshot_filename = Path::new(&screenshot_file)
            .file_name()
            .unwrap()
            .to_string_lossy();
        let thumbnail_filename = Path::new(&thumbnail_file)
            .file_name()
            .unwrap()
            .to_string_lossy();
...
fn generate_random_timestamps(duration: f64, count: usize) -> Vec<u32> {
    ...
    let mut timestamps: Vec<u32> = (0..count)
        .map(|_| rng.gen_range(start_time..end_time))
        .collect();
    timestamps.sort();
    timestamps
}
...
fn generate_screenshot(
    ...
    if !status.success() {
        return Err(SeedError::Validation(format!(
            "ffmpeg screenshot generation failed"
        )));
    }
